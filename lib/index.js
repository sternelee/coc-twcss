...
var items = {
  ...
  "divide-x-2 > * + *": "border-right-width: 2px;\nborder-left-width: 0px;",
  "divide-x-4 > * + *": "border-right-width: 4px;\nborder-left-width: 0px;",
  "divide-x-8 > * + *": "border-right-width: 8px;\nborder-left-width: 0px;",
  "divide-x > * + *": "border-right-width: 1px;\nborder-left-width: 0px;",
  "divide-y-0 > * + *": "border-top-width: 0px;\nborder-bottom-width: 0px;",
  "divide-y-2 > * + *": "border-top-width: 0px;\nborder-bottom-width: 2px;",
  "divide-y-4 > * + *": "border-top-width: 0px;\nborder-bottom-width: 4px;",
  "divide-y-8 > * + *": "border-top-width: 0px;\nborder-bottom-width: 8px;",
...
// src/index.ts
var items2 = Object.keys(twcssItems).map((k) => ({ word: k, info: twcssItems[k], kind: "TW" }));
var isTwind = false;
var isUnoCSS = false;
async function activate(context) {
  const isEnabled = import_coc.workspace.getConfiguration("coc-twcss").get("enabled", false);
  isTwind = import_coc.workspace.getConfiguration("coc-twcss").get("twind", false);
  isUnoCSS = import_coc.workspace.getConfiguration("coc-twcss").get("unocss", false);
  if (!isEnabled)
    return;
  ...
  context.subscriptions.push(
    import_coc.sources.createSource({
      ...
      doComplete: async () => {
        let items3 = await getCompletionItems();
        if (isTwind) {
          items3 = items3.map((v) => ({ ...v, info: `Twind: 
 ${v.info}` }));
        }
        if (isUnoCSS) {
          items3 = items3.map((v) => ({ ...v, info: `UnoCSS: 
 ${v.inf}` }));
        }
        return items3;
      }
